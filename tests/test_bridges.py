...
def test_targets_listing_for_personnel(admin_client):
    r = admin_client.get("/api/v1/bridge/personnel_issues/targets")
...
def test_targets_listing_hides_sensitive_bridges_from_non_admin(auth_client):
    r = auth_client.get("/api/v1/bridge/personnel_issues/targets")
    assert r.status_code == 200
    assert r.get_json() == []

    r = auth_client.get("/api/v1/bridge/work_tasks/targets")
    assert r.status_code == 200
    targets = {row["target"] for row in r.get_json()}
    assert "personnel_issues" not in targets


def test_bridge_personnel_requires_admin(auth_client):
    src = auth_client.post("/api/v1/personnel_issues", json={
        "person_name": "Restricted",
        "issue_description": "sensitive narrative",
    }).get_json()
...
def test_bridge_to_personnel_requires_admin(auth_client):
    src = auth_client.post("/api/v1/work_tasks", json={
        "title": "Capability follow-up",
        "description": "should not become an incident without admin",
    }).get_json()

    r = auth_client.post(
        f"/api/v1/bridge/work_tasks/{src['id']}/personnel_issues",
        json={"overrides": {"person_name": "Someone"}},
    )
    assert r.status_code == 403
    assert "admin" in r.get_json()["error"].lower()
...
def test_bridge_personnel_to_training(admin_client, temp_app):
    src = admin_client.post("/api/v1/personnel_issues", json={
...
def test_bridge_writes_dual_activity_log(admin_client, temp_app):
    src = admin_client.post("/api/v1/personnel_issues", json={
        "person_name": "Bob",
...
def test_bridge_personnel_to_work(admin_client, temp_app):
    src = admin_client.post("/api/v1/personnel_issues", json={
        "person_name": "Eve",
...
def test_bridge_work_to_personnel_requires_override(admin_client):
    src = admin_client.post("/api/v1/work_tasks", json={
...
def test_bridge_work_to_personnel_with_override(admin_client, temp_app):
    src = admin_client.post("/api/v1/work_tasks", json={
        "title": "Big rendering bug",
        "description": "Renderer barfs on long sheet sets",
        "cad_skill_area": "Sheet Production",
...
def test_bridge_training_to_personnel(admin_client, temp_app):
    src = admin_client.post("/api/v1/training_tasks", json={
...
def test_bridge_idempotent_with_key(admin_client):
    src = admin_client.post("/api/v1/personnel_issues", json={
...
def test_bridge_no_key_creates_separate_rows(admin_client):
    src = admin_client.post("/api/v1/personnel_issues", json={
...
def test_bridge_unknown_target_table(admin_client):
    src = admin_client.post("/api/v1/personnel_issues", json={
...
def test_bridge_missing_source_id(admin_client):
    r = admin_client.post("/api/v1/bridge/personnel_issues/99999/training_tasks")
