...
class ProjectWorkTask(Base):
    ...
    scheduled_completion_at: Mapped[str] = mapped_column(Text, server_default=text("''"))
    time_required_minutes: Mapped[int] = mapped_column(Integer, server_default=text("0"))
    notes: Mapped[str] = mapped_column(Text, server_default=text("''"))
    scope_notes: Mapped[str] = mapped_column(Text, server_default=text("''"))
    progress_notes: Mapped[str] = mapped_column(Text, server_default=text("''"))
    confirmation_notes: Mapped[str] = mapped_column(Text, server_default=text("''"))
    completion_notes: Mapped[str] = mapped_column(Text, server_default=text("''"))
    created_by_user_id: Mapped[int | None] = mapped_column(Integer)
    created_by_name: Mapped[str] = mapped_column(Text, server_default=text("''"))
    created_at: Mapped[datetime] = mapped_column(TIMESTAMP, server_default=text("CURRENT_TIMESTAMP"))
    updated_at: Mapped[datetime] = mapped_column(TIMESTAMP, server_default=text("CURRENT_TIMESTAMP"))
